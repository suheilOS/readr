...
12. **Browser capture** — historical first slice: Chrome MV3 capture of a YouTube transcript, same-origin persistence through the signed-in Readr tab, D1 media content, and reader-first stored fallback. The current one-click extension and live-page Defuddle flow are tracked in Phase 3 of `capture-roadmap.md`; article capture, OAuth, cross-browser packaging, and broader provider work remain out of scope.
